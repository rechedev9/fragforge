...
The integrated agent can use only the dynamic `fragforge` namespace backed by Studio's typed operation gateway: it searches live operations first, executes reads directly, and turns writes, costly work, and destructive operations into Studio approval previews. It can import public Twitch URLs and can open a native Studio picker for local demos or stream recordings; selected paths remain main-process-only and never enter model context. Voice files remain in their existing Studio picker. After an approved operation, the agent resumes automatically with its safe result, and it can register bounded background state watches so parsing, capture, analysis, and render transitions wake the next agent turn without requiring the user to poll.
Capture and render therefore have two separate gates: approval of the complete creative brief, followed by approval of the exact operation preview. Stream briefs are derived from and fingerprint the canonical saved edit plan; any plan mutation or fingerprint mismatch invalidates both the brief and prepared render previews. Stream render admission also checks the approved plan timestamp atomically under the backend's plan lock, closing the final edit-versus-render race.

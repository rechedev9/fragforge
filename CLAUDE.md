...
when changing project-wide agent behavior.
...
Repo-local operational safety rules are also mirrored in
...
Repo-local slash commands live under `.claude/commands/`.
...
Repo-local reviewer agents live under `.claude/agents/`. Use them by name:

- `@go-readability-reviewer review the current diff`
- `@go-test-reviewer review the tests changed in this diff`
...
Non-interactive wrappers live under `scripts/claude-zv-*.sh` and call Claude Code
print mode with the same playbooks.

Use `scripts/claude-run.sh .claude/commands/zv-tdd.md "custom prompt run"` for
a direct command-file run. Use `CLAUDE_DRY_RUN=1` to preview the final prompt
without calling Claude Code.

Common wrappers:

- `scripts/claude-zv-tdd.sh "implement ..."` for behavior changes.
- `scripts/claude-zv-bugfix.sh "fix ..."` for bugs.
- `scripts/claude-zv-pr-ready.sh` before review/PR.

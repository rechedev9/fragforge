// Release evals for FragForge Studio 2.2.10. This launches the real Electron
...
const artifactsDir = join(desktopRoot, 'e2e', 'artifacts', 'release-2.2.10');
...
test('installed release exposes Agent-only desktop settings and consistent navigation', async () => {
  ...
  try {
    ...
    assert.equal(values[versionIndex], '2.2.10');
    await installedPage.screenshot({ path: join(artifactsDir, 'installed-settings-2.2.10.png'), fullPage: true });
  } finally {

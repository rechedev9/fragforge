...
interface RenderVariantState {
  variant?: string;
  status?: string;
  preset?: string;
  edit_document_key?: string;
  edit_manifest_key?: string;
  render_result_key?: string;
  pack_manifest_key?: string;
  gallery_key?: string;
  publish_summary_key?: string;
...
type RailTab = "codex" | "inspector" | "artifacts";
...
interface AppState {
  ...
  renderState: RenderVariantState | null;
  activeTab: RailTab;
  lastMomentsError: string;
...
let selectedDemoFile: File | null = null;
let refreshInFlight = false;
let refreshTimer: number | undefined;

const stages = ["Intake", "Parse", "Plan", "Record", "Render", "Validate", "Publish"] as const;
...
const dom = {
  apiChip: required<HTMLSpanElement>("api-chip"),
  artifactLinks: required<HTMLDivElement>("artifact-links"),
  actionProposal: required<HTMLDivElement>("action-proposal"),
  codexCommand: required<HTMLInputElement>("codex-command"),
  codexThread: required<HTMLDivElement>("codex-thread"),
  commandForm: required<HTMLFormElement>("command-form"),
  demoFile: required<HTMLInputElement>("demo-file"),
  demoFileName: required<HTMLSpanElement>("demo-file-name"),
  dropZone: required<HTMLLabelElement>("drop-zone"),
  health: required<HTMLSpanElement>("health"),
  inspectorJson: required<HTMLPreElement>("inspector-json"),
  jobCount: required<HTMLSpanElement>("job-count"),
  jobs: required<HTMLDivElement>("jobs"),
  momentCount: required<HTMLSpanElement>("moment-count"),
  moments: required<HTMLDivElement>("moments"),
  previewSubtitle: required<HTMLSpanElement>("preview-subtitle"),
  previewTimecode: required<HTMLSpanElement>("preview-timecode"),
  previewTitle: required<HTMLSpanElement>("preview-title"),
  railArtifacts: required<HTMLDivElement>("rail-artifacts"),
  refresh: required<HTMLButtonElement>("refresh"),
  runPrompt: required<HTMLTextAreaElement>("run-prompt"),
  stageTimeline: required<HTMLDivElement>("stage-timeline"),
  startRun: required<HTMLButtonElement>("start-run"),
  targetSteamID: required<HTMLInputElement>("target-steamid"),
  token: required<HTMLInputElement>("token"),
  workspaceStatus: required<HTMLDivElement>("workspace-status"),
};
...
function setHealth(message: string, kind: "ok" | "warn" | "bad" = "ok"): void {
...
function mutationHeaders(json = false): Headers {
...
function setSelectedDemoFile(file: File | null): void {
  selectedDemoFile = file;
  dom.demoFileName.textContent = file?.name || "No file selected";
}

function statusProgress(status?: JobStatus): number {
  switch (status) {
    case "queued":
      return 8;
    case "parsing":
...
function momentEventLabel(moment: Moment): string {
...
function renderStatusChips(job: Job | null): void {
  ...
  dom.workspaceStatus.append(
    text("span", "chip mono", fileName(job.demo_path)),
    statusBadge(job.status),
    text("span", "chip mono", elapsedLabel(job)),
    variantSelect,
    text("span", "chip good", "OK"),
  );
}
...
function renderJobs(): void {
  ...
  for (const job of state.jobs) {
    ...
    middleLine.append(
      text("span", "run-subtitle mono", shortID(job.id)),
      text("span", "run-subtitle truncate", job.target_steamid || "target pending"),
    );
...
function stageState(stage: (typeof stages)[number], job: Job | null): "done" | "active" | "pending" {
  if (!job) {
    return stage === "Intake" ? "active" : "pending";
  }
  const status = job.status;
  const renderStatus = state.renderState?.status;
  const done = (names: string[]) => names.includes(stage);
...
function renderTimeline(job: Job | null): void {
...
function renderPreview(job: Job | null): void {
  if (!job) {
...
function renderMoments(job: Job | null): void {
  ...
  for (const moment of state.moments.slice(0, 40)) {
    ...
    row.append(
      text("td", "mono", formatSeconds(moment.time_start_seconds)),
      text("td", "", momentEventLabel(moment)),
      text("td", "", moment.player || "unknown"),
      text("td", (moment.score || 0) >= 0.9 ? "score-good" : "", formatScore(moment.score)),
    );
...
function artifactLinks(job: Job | null): ArtifactLink[] {
  if (!job) {
    return [];
  }
  const id = job.id;
...
function renderArtifacts(job: Job | null): void {
  ...
  for (const link of links) {
    ...
    if (link.href) {
      (chip as HTMLAnchorElement).href = link.href;
    }
...
function currentProposal(job: Job | null): Proposal {
  if (!job) {
    return {
      code: "START_NEW_RUN",
...
function renderCodex(job: Job | null): void {
  ...
  dom.actionProposal.append(
    text("div", "proposal-code mono", proposal.code),
    text("p", "proposal-copy", proposal.description),
  );
...
function message(kind: "codex" | "user", title: string, copy: string): HTMLElement {
...
function renderTabs(): void {
  ...
  for (const tab of ["codex", "inspector", "artifacts"] as RailTab[]) {
    required<HTMLElement>(`tab-${tab}`).classList.toggle("active", tab === state.activeTab);
  }
}
...
async function refresh(options: { silent?: boolean } = {}): Promise<void> {
  ...
  try {
    ...
    const [jobsResponse, loadoutsResponse] = await Promise.all([
      api<JobsResponse>("/api/jobs?limit=50"),
      api<LoadoutsResponse>("/api/loadouts"),
    ]);
...
async function loadSelectedDetails(): Promise<void> {
  ...
  const [momentsResult, renderResult] = await Promise.allSettled([
    api<MomentsResponse>(`/api/jobs/${job.id}/moments`),
    api<RenderVariantState>(`/api/jobs/${job.id}/renders/${selectedVariant()}`),
  ]);
...
async function createRun(): Promise<void> {
  ...
  try {
    ...
    const created = await api<{ id: string; status: string }>("/api/jobs", init);
...
function wireEvents(): void {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-tab]").forEach((button) => {
    button.addEventListener("click", () => setTab((button.dataset.tab || "codex") as RailTab));
  });

	...
	return []productionFlow{
		...
		},
		{
			...
			Phases: []flowPhase{
				...
				{ID: "creative-brief", Goal: "ask only unanswered stream edit questions and receive explicit approval before rendering; ambiguous words like go/hazlo are not approval until they answer a shown brief", Decision: "layout; clip boundaries/title; clean crop/framing; killfeed source or re-overlay; Spanish captions and review policy; music; delivery shape; cover strategy", Produces: "approved stream brief and exact render choices", Gate: true, ReadOnly: true},
				{ID: "plan-preflight", Goal: "probe media and preview the clip/crop/caption contract without writing", Command: "zv stream plan --input <stream.mp4> --out <run>/edit-plan.json --variant <" + strings.Join(streamclips.VariantNames(), "|") + "> [--killfeed-crop <x,y,w,h> --detect-killfeed] [--captions] --dry-run --format json", Decision: "clip ranges, clean crops, killfeed policy, and subtitle source", ReadOnly: true, Expensive: true},
				{ID: "plan", Goal: "persist the approved stream edit contract", Command: "zv stream plan --input <stream.mp4> --out <run>/edit-plan.json --variant <" + strings.Join(streamclips.VariantNames(), "|") + "> [--killfeed-crop <x,y,w,h> --detect-killfeed] [--captions] --format json", Produces: "edit-plan.json", Expensive: true},

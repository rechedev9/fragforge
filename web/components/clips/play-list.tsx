 ...
 * header with the selection count plus Select all / Clear, and no horizontal
 * scroll at any width.
 */
    ...
    <div className="flex flex-col overflow-hidden rounded-xl border border-border">
      <div className="flex items-center justify-between gap-3 border-b border-border/70 bg-muted/30 px-3 py-2">
        ...
        <div className="flex items-center gap-1">
          <Button type="button" variant="ghost" size="xs" disabled={allSelected} onClick={onSelectAll}>
            Select all
          </Button>
          <Button type="button" variant="ghost" size="xs" disabled={selectedIds.size === 0} onClick={onClear}>
            Clear
          </Button>
        </div>
      </div>
